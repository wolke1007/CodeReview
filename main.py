...
class main():
    def run(self):
        ...
        # 檢查 Controller
        ControllerPage(file_path=controller_file_path, controller_name=controller_name).set_all_rules_to_check().check_all_rules()
        # 檢查 Service
        ...
        for service_file_path in service_file_paths:
            ServicePage(file_path=service_file_path, controller_name=controller_name).set_all_rules_to_check().check_all_rules()
        # 檢查 ServiceImpl
        serviceimpl_file_paths = get_serviceimpl_file_paths(service_names)
        for serviceimpl_file_path in serviceimpl_file_paths:
            ServiceImplPage(file_path=serviceimpl_file_path, controller_name=controller_name).set_all_rules_to_check().check_all_rules()
        # 檢查 Dao
        ...
        for dao_file_path in dao_file_paths:
            DaoPage(file_path=dao_file_path, controller_name=controller_name).set_all_rules_to_check().check_all_rules()
