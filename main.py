...
def main(file_path):
    ...
    # 檢查 Controller
    page.ControllerPage(file_path=controller_file_path,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        page_pool = {"ServicePage": page.ServicePage,
                     "ControllerPage": page.ControllerPage,
                     "DaoPage": page.DaoPage,
                     "ServiceImplPage": page.ServiceImplPage,
                     "JspPage": page.JspPage}
        page_obj = page_pool.get(sys.argv[2])
        if page_obj is None:
            print("沒有此種類型的 page: " + str(sys.argv[2]))
            exit()
        project_root_path = util.get_project_root_path()
        controller_name = re.search("/\w*Controller\.java", sys.argv[1])
        controller_name = controller_name.group(
        )[1:-5] if controller_name else None
        page_obj = page_obj(project_root_path + "/" +
                            sys.argv[1], controller_name)
